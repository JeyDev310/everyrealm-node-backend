...
export class SendVerificationEmailService extends ServiceBase {
  get constraints () {
    return constraints
  }
  ...
  async run () {
    ...
    try {
      const emailTemplate = await this.context.sequelize.models.emailTemplate.findOne({ where: { eventType: EMAIL_TEMPLATE_EVENT_TYPES.EMAIL_VERIFICATION, isDefault: true }, transaction: this.context.sequelizeTransaction })
      ...
      if (!emailTemplate) {
        template = `
          ...
          <li>${settings[SETTING_KEYS.USER_END_URL]}/verify-email?token=${token}<li>
        `
      } else {
        const replaceableData = {
          link: `${settings[SETTING_KEYS.USER_END_URL]}/verify-email?token=${token}`,
      ...
      const emailVerificationSent = await sendEmail(this.args.email, this.args.name || this.args.email, 'Arc email verification', template)
